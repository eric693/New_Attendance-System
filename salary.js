...
// salary-enhanced.js - 薪資管理前端邏輯（完整版 - 包含職務加給、請假扣款、伙食費、車補費、福利金等）
...
async function handleSalaryConfigSubmit(e) {
    ...
    try {
        ...
        const res = await callApifetch(`setEmployeeSalaryEnhanced&${queryString}`);
...
async function handleSalaryCalculation() {
    ...
    try {
        ...
        const res = await callApifetch(`calculateMonthlySalaryEnhanced&employeeId=${encodeURIComponent(employeeId)}&yearMonth=${encodeURIComponent(yearMonth)}`);
...
function displaySalaryCalculation(data, container) {
    ...
    container.innerHTML = `
        <div class="feature-box" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(5, 150, 105, 0.1) 100%);">
            <h3 class="text-xl font-bold mb-4">
                ${data.employeeName || '--'} - ${data.yearMonth || '--'} 薪資計算結果
            </h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div style="background: rgba(255, 255, 255, 0.03); padding: 1.5rem; border-radius: 12px;">
                    ...
                    <div style="display: flex; justify-content: space-between; padding: 1rem 0; margin-top: 0.5rem; border-top: 2px solid rgba(16, 185, 129, 0.3); font-size: 1.25rem; font-weight: 700; color: #10b981;">
                        <span>應發總額</span>
                        <span>${formatCurrency(data.grossSalary)}</span>
                    </div>
                </div>
...
async function saveSalaryRecord(data) {
    try {
        ...
        const res = await callApifetch(`saveMonthlySalaryEnhanced&${queryString}`);
...
async function loadAllEmployeeSalaryFromList() {
    ...
    try {
        ...
        const res = await callApifetch(`getAllMonthlySalaryEnhanced&yearMonth=${encodeURIComponent(yearMonth)}`);
