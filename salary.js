            ...
            <div class="mt-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                    <div>
                        ...
                        <span class="font-mono">${formatCurrency(data.employerLaborFee || 0)}</span>
                    </div>
                    <div>
                        <span class="text-gray-600 dark:text-gray-400">健保費：</span>
                        <span class="font-mono">${formatCurrency(data.employerHealthFee || 0)}</span>
                    </div>
                    <div>
                        <span class="text-gray-600 dark:text-gray-400">就保費：</span>
                        <span class="font-mono">${formatCurrency(data.employerEmploymentFee || 0)}</span>
                    </div>
                    <div>
                        <span class="text-gray-600 dark:text-gray-400">勞退提繳：</span>
                        <span class="font-mono">${formatCurrency(data.employerPension || 0)}</span>
                    </div>
                </div>
            </div>
        </div>
...
function getBankName(code) {
    const banks = {
        ...
        "012": "台北富邦", "013": "國泰世華", "017": "兆豐銀行",
        "803": "聯邦銀行", "806": "元大銀行", "807": "永豐銀行",
        "808": "玉山銀行", "809": "凱基銀行", "812": "台新銀行",
        "822": "中國信託"
    };
    return banks[code] || code || '--';
}
